...
class MovementService:
    ...
    def estimate_explorer_movement(self,agent):
        if in_circle(agent.x,agent.y,agent.exploration_centroid[0],agent.exploration_centroid[1],agent.exploration_buffer_radius):
            ...
            new_x = agent.x + agent.kappa*(x_centroid-agent.x)+agent.sigma*np.random.randn()[0]
            new_y = agent.y + agent.kappa*(y_centroid-agent.y)+agent.sigma*np.random.randn()[0]
            ...
            while not any(constraints):                                        
                ...
                grad_x = torch.autograd.grad(constraint_functions,new_x,retain_graph=True)[0]
                grad_y = torch.autograd.grad(constraint_functions,new_y)[0]
                new_x = new_x - 0.1*grad_x
                new_y = new_y - 0.1*grad_y
                constraints = [self.max_speed_constraint_condition(agent,agent.x,agent.y,new_x.to_numpy()[0],new_y.to_numpy()[0]),
                               self.exploration_movement_condition(agent,new_x.to_numpy()[0],new_y.to_numpy()[0])]
        else:
    ...
    def estimate_communicator_movement(self,agent,agents):
        ...
        new_x = agent.x + agent.kappa*attraction_x+agent.sigma*np.random.randn()[0]
        new_y = agent.y + agent.kappa*attraction_y+agent.sigma*np.random.randn()[0]
        new_x,new_y = self.constraint_movement(agent,agent.x,agent.y,new_x,new_y)
